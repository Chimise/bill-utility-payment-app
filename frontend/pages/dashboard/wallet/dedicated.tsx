...
const DedicatedAccountPage = () => {
    ...
    return (<div className='bg-white w-full h-auto md:h-full'>
        <UtilityHeader title="Fund Your Account" />
        <div className='p-7  my-3 mb-5'>
        <p className='mb-4 text-slate-900 text-sm font-light text-center'>Send Money to any of the account and top up instantly</p>
        <div className='w-full space-y-8 md:space-y-0 md:flex md:space-x-7 md:justify-center'>
          {error && <div className='flex justify-center p-3'>
            <Error error={error.message} onRetry={() => mutate()} className="p-5" />
            </div>}

          {isLoading && <div className='flex justify-center p-3'>
            <Spinner className='p-2' />
            </div>
            }
        {accounts && accounts.map(account => (<Paper key={account.id}>
        <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <div className="text-sm font-medium text-gray-500">Bank</div>
            <div className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{account.bank}</div>
          </div>
          <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <div className="text-sm font-medium text-gray-500">Account Name</div>
            <div className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{account.name}</div>
          </div>
          <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <div className="text-sm font-medium text-gray-500">Account Number</div>
            <div className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{account.account_number}</div>
          </div>
          <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <div className="text-sm font-medium text-gray-500">Amount</div>
            <div className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{total}</div>
          </div>
        </Paper>))}
    </div>
        </div>
